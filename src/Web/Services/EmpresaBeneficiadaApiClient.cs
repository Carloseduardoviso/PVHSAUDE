...
public class EmpresaBeneficiadaApiClient(HttpClient client)
{
    ...
    {
        return await client.GetFromJsonAsync<List<CredenciadoVm>>("api/empresas-beneficiadas", ct) ?? [];
    }

    public async Task<CredenciadoVm?> ObterAsync(Guid id, CancellationToken ct)
    {
        using var response = await client.GetAsync($"api/empresas-beneficiadas/{id}", ct);
    ...
    public async Task SalvarAsync(CredenciadoVm model, CancellationToken ct)
    {
        using var response = model.Id == Guid.Empty
            ? await client.PostAsJsonAsync("api/empresas-beneficiadas", model, ct)
            : await client.PutAsJsonAsync($"api/empresas-beneficiadas/{model.Id}", model, ct);
        await Verificar(response, ct);
        if (model.Id == Guid.Empty)
            model.Id = (await response.Content.ReadFromJsonAsync<CredenciadoVm>(ct))?.Id
    ...
    public async Task UploadImagemAsync(Guid id, IFormFile imagem, CancellationToken ct)
    {
        ...
        using var response = await client.PostAsync($"api/empresas-beneficiadas/{id}/imagem", content, ct);
