...
public class BannerApiClient(HttpClient http)
{
    public async Task<List<BannerViewModel>> ListarAsync(bool ativos, CancellationToken ct) =>
        await http.GetFromJsonAsync<List<BannerViewModel>>(ativos ? "api/banners/ativos" : "api/banners", ct) ?? [];
    ...
    public async Task<string?> SalvarAsync(BannerViewModel model, CancellationToken ct)
    {
        ...
        using var request = new HttpRequestMessage(model.Id == Guid.Empty ? HttpMethod.Post : HttpMethod.Put,
            model.Id == Guid.Empty ? "api/banners" : $"api/banners/{model.Id}") { Content = content };
