...
public class CatalogoController(IHttpClientFactory factory) : Controller
{
    ...
    [HttpGet] public async Task<IActionResult> Especialidades(CancellationToken ct) => View("Especialidades", await Api().GetFromJsonAsync<List<CatalogoItemViewModel>>("api/especialidades", ct) ?? []);
    ...
    [HttpGet] public async Task<IActionResult> Procedimentos(CancellationToken ct) => View("Procedimentos", await Api().GetFromJsonAsync<List<CatalogoItemViewModel>>("api/procedimentos", ct) ?? []);
