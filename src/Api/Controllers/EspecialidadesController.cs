...
public class EspecialidadesController(Context db) : ControllerBase
{
 ...
 [HttpPost] public async Task<IActionResult> Post(CatalogoRequest r,CancellationToken ct){var x=new Especialidade(r.Nome);db.Especialidades.Add(x);await db.SaveChangesAsync(ct);return Ok(new EspecialidadeResponse(x.Id,x.Nome,x.Ativo));}
}
