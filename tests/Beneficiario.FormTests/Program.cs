...
var sobreNosResponse = await client.GetAsync("/Home/SobreNos");
if (!sobreNosResponse.IsSuccessStatusCode) throw new Exception($"Sobre nós failed: {(int)sobreNosResponse.StatusCode}");
var sobreNosHtml = WebUtility.HtmlDecode(await sobreNosResponse.Content.ReadAsStringAsync());
